...
function HallManager({state,setState}){
  ...
  function patchExact(id,field,value){setState(q=>{const snapshot=rawCareerTournamentStats(q,id,ensemble),at=new Date().toISOString(),player=q.students.find(x=>x.id===id);return{...q,students:q.students.map(s=>{if(s.id!==id)return s;const current={...(s.hallAdjustments?.[ensemble]||{})},allBaselines={...(s.careerBaselines||{})},baseline={...(allBaselines[ensemble]||{})},recordedAtEdit={...(baseline.recordedAtEdit||{})},editedAt={...(baseline.editedAt||{})};if(value===""){delete current[field];delete recordedAtEdit[field];delete editedAt[field];}else{current[field]=Math.max(0,Number(value)||0);recordedAtEdit[field]=field==="titles"||field==="defenses"?snapshot.titles:Number(snapshot[field]||0);editedAt[field]=at;}allBaselines[ensemble]={...baseline,recordedAtEdit,editedAt};return{...s,hallAdjustments:{...(s.hallAdjustments||{}),[ensemble]:current},careerBaselines:allBaselines};}),meta:{...q.meta,lastHistoryEdit:{ensemble,text:(player?.name||"Player")+" "+field+" baseline "+(value===""?"returned to recorded history":"set to "+Math.max(0,Number(value)||0)),at}}};});}
